...
--
-- Si sale avisan = 0, el `update` no encajó con ninguna fila: la secuencia no
-- estaba como la dejó el refundido (se editó en el panel, o ya se corrigió).
-- Si sale botones = 2, lo que hay es el seed original, sin refundir.
--
-- `left join lateral ... on true`, no `cross join lateral`: con el cross, una
-- secuencia cuyo paso de inicio se quedara sin botones DESAPARECERÍA del
-- resultado en vez de salir con ceros, y «no aparece» se lee como «no existe»
-- en vez de como el problema que es.
select s.nombre,
       count(b.valor) filter (where b.valor->'ruta' ? 'ir_a')                as botones,
       count(b.valor) filter (where (b.valor->'ruta'->>'avisar')::boolean)   as avisan,
       count(b.valor) filter (where b.valor->'ruta'->>'fase' = 'interesado') as con_fase,
       count(b.valor) filter (where b.valor->'ruta' ? 'terminar')            as con_terminar
from public.ventas_secuencias s
join public.ventas_marcas m on m.id = s.marca_id
left join lateral jsonb_array_elements(s.pasos->'pasos'->(s.pasos->>'inicio')->'botones') as b(valor)
  on true
